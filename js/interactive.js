...
const createBallsFromColors = (colors = []) => {
  ...
  return colors.map(color => {
    const myShape = animateShape(protoCircle, 0, 0);
    myShape.color = color;
    return myShape;
  });
};
...
ranges('ballSize').value = theseBalls[0].side;

function startListeners() {
  ranges('ballSize')
    .addEventListener('input', (range) => changeSize(range.target.value, theseBalls));
  ranges('ballSpeedX')
    .addEventListener('input', (range) => changeSpeedX(range.target.value, theseBalls));
  ranges('ballSpeedY')
    .addEventListener('input', (range) => changeSpeedY(range.target.value, theseBalls));
}
